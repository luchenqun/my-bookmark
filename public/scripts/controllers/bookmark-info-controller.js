app.controller('bookmarkInfoCtr', [
  '$scope',
  '$state',
  '$timeout',
  '$sce',
  ...
  'pubSubService',
  'dataService',
  function ($scope, $state, $timeout, $sce, $window, $filter, $document, $timeout, pubSubService, dataService) {
    ...
    pubSubService.subscribe('TagCtr.showBookmarkInfo', $scope, async function (event, bookmark) {
      ...
      $('.ui.modal.js-bookmark-info')
        .modal({
          closable: false
        })
        .modal('setting', 'transition', dataService.animation())
        .modal('show');
      bookmark.favicon_url = '/favicon?url=' + bookmark.url;
