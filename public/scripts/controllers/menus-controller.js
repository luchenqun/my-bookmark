app.controller('menuCtr', [
  ...
  'pubSubService',
  'dataService',
  function ($scope, $stateParams, $state, $window, $timeout, $document, pubSubService, dataService) {
    ...
    async function updateMenuActive() {
      ...
      let activeIndex = mapIndex[document.location.hash];
      console.log('updateMenuActive', $scope.login, document.location.hash, activeIndex);

      $scope.selectLoginIndex = activeIndex;
      $scope.selectNotLoginIndex = activeIndex;
      $('.ui.menu a.item').removeClass('selected');
      if (Number.isInteger(activeIndex) && activeIndex >= 0) {
        $('.ui.menu a.item:eq(' + activeIndex + ')').addClass('selected');
      }
    }
    ...
    $timeout(function () {
      $('.suggest').popup({
        ...
        variation: 'very wide',
        html: "<span><span class='fontred'>特别提示：对照更新日志，如果有更新，请按 Ctrl+F5 强制更新或者清理浏览器缓存！<br/>点击该按钮即可查看更新日志！</span><br/>1、在任意页面，按A键添加备忘录。<br/>2、在任意页面，按数字键切换菜单栏。<br/>3、在书签页面鼠标放在书签上，按C复制书签链接<br/>4、在书签页面鼠标放在书签上，按E编辑书签<br/>5、在书签页面鼠标放在书签上，按D删除书签<br/>6、在书签页面鼠标放在书签上，按I查看书签详情<br/>7、在任意页面，按INSERT做添加书签<br/>8、在任意页面，按ESC退出弹窗<br/></span>"
      });
    }, 1000);
      ...
      $scope.$apply(async function () {
        ...
        if (dataService.keyShortcuts()) {
          ...
          if (key == ',' || key == '.' || key == '/') {
            ...
            $state.go(dataService.loginMenus[dataService.LoginIndexTags].uiSref, stateParams, {
              reload: true
            });
          }
          ...
          if (!isNaN(key)) {
            ...
            $state.go(
              dataService.loginMenus[num - 1].uiSref,
              {},
              {
                reload: true
              }
