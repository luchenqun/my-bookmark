    ...
    (e.fn.calendar = function (t) {
      ...
      return (
        r.each(function () {
          ...
          ((c = {
            initialize: function () {
              (c.debug('Initializing calendar for', H),
                (r = c.get.isTouch()),
                c.setup.popup(),
                c.setup.inline(),
                c.setup.input(),
            ...
            destroy: function () {
              (c.verbose('Destroying previous calendar for', H),
                w.removeData(M),
                c.unbind.events());
            },
            setup: {
              popup: function () {
                if (!f.inline && (T.length || ((T = w.children().first()), T.length))) {
                  ...
                  (k.length || (k = e('<div/>').addClass(g.popup).prependTo(T.parent())),
              ...
              inline: function () {
                (T.length && !f.inline) ||
                  ((k = e('<div/>').addClass(g.calendar).appendTo(w)),
            ...
            create: {
              calendar: function () {
                ...
                var E = C ? 10 : M ? 1 : 0,
                  ...
                  A = x || T ? 1 : 0,
                  S = x || T ? m : 1,
                  j = new Date(b - E, v - R, S - A, h),
                  V = new Date(b + E, v + R, S + A, h),
                  P = C
                    ? new Date(10 * Math.ceil(b / 10) - 9, 0, 0)
                    : M
                      ? new Date(b, 0, 0)
                      : w
                        ? new Date(b, v, 0)
                        : new Date(b, v, m, -1),
                  q = C
                    ? new Date(10 * Math.ceil(b / 10) + 1, 0, 1)
                    : M
                      ? new Date(b + 1, 0, 1)
                      : w
                        ? new Date(b, v + 1, 1)
                ...
                if (!H) {
                  ...
                  (L.data(D.focusDate, j),
                    L.toggleClass(g.disabledCell, !c.helper.isDateInRange(P, i)),
                    e('<i/>').addClass(g.prevIcon).appendTo(L));
                  var B = e('<span/>').addClass(g.next).appendTo(r);
                  if (
                    (B.data(D.focusDate, V),
                    B.toggleClass(g.disabledCell, !c.helper.isDateInRange(q, i)),
                    e('<i/>').addClass(g.nextIcon).appendTo(B),
                    w)
                  )
                    for (o = e('<tr/>').appendTo(K), t = 0; t < F; t++)
                      ((r = e('<th/>').appendTo(o)),
                        r.text(y.dayColumnHeader((t + f.firstDayOfWeek) % 7, f)));
                }
                    ...
                    var Q = C
                        ...
                        : M
                          ...
                          : w
                            ...
                            : x
                              ? new Date(b, v, m, t)
                              : new Date(b, v, m, h, 5 * t),
                      Z = C ? t : M ? f.text.monthsShort[t] : w ? Q.getDate() : y.time(Q, f, !0);
                    ((r = e('<td/>').addClass(g.cell).appendTo(o)), r.text(Z), r.data(D.date, Q));
                    var G = w && Q.getMonth() !== v,
                      X = G || !c.helper.isDateInRange(Q, i),
                      $ = c.helper.dateEqual(Q, u, i);
                    (r.toggleClass(g.adjacentCell, G),
                      r.toggleClass(g.disabledCell, X),
                      r.toggleClass(g.activeCell, $),
                      x || T || r.toggleClass(g.todayCell, c.helper.dateEqual(Q, l, i)),
                      c.helper.dateEqual(Q, d, i) && c.set.focusDate(Q, !1, !1));
                  }
                if (f.today) {
                  var _ = e('<tr/>').appendTo(U),
                    ee = e('<td/>')
                      .attr('colspan', '' + F)
                      .addClass(g.today)
                      .appendTo(_);
                  (ee.text(y.today(f)), ee.data(D.date, l));
                }
                ...
                a.find('td').each(function () {
                  ...
                  if (a) {
                    var o = t.hasClass(g.disabledCell),
                      ...
                      f =
                        !!d &&
                        ((!!l && c.helper.isDateInRange(a, n, l, d)) ||
                          (!!u && c.helper.isDateInRange(a, n, d, u)));
                    (t.toggleClass(g.focusCell, p && (!r || I)),
                      t.toggleClass(g.rangeCell, f && !s && !o));
                  }
                });
              }
            },
            ...
            bind: {
              events: function () {
                (k.on('mousedown' + C, c.event.mousedown),
                  ...
                  x.length
                    ? (x.on('input' + C, c.event.inputChange),
                      x.on('focus' + C, c.event.inputFocus),
                      x.on('blur' + C, c.event.inputBlur),
                      x.on('click' + C, c.event.inputClick),
                      x.on('keydown' + C, c.event.keydown))
                    : k.on('keydown' + C, c.event.keydown));
              }
            },
            unbind: {
              events: function () {
            ...
            event: {
              ...
              keydown: function (e) {
                if (
                  ((27 !== e.keyCode && 9 !== e.keyCode) || c.popup('hide'), c.popup('is visible'))
                )
                  if (
                    37 === e.keyCode ||
                    38 === e.keyCode ||
                    ...
                    ('time' === f.type && (s = c.helper.mergeDateTime(o, s)),
                      c.helper.isDateInRange(s, t) && c.set.focusDate(s));
            ...
            get: {
              ...
              validModes: function () {
                ...
                return (
                  'time' !== f.type &&
                    ((f.disableYear && 'year' !== f.type) || e.push('year'),
                    ((!f.disableMonth && 'year' !== f.type) || 'month' === f.type) &&
                      e.push('month'),
                    f.type.indexOf('date') >= 0 && e.push('day')),
                  f.type.indexOf('time') >= 0 &&
                    (e.push('hour'), f.disableMinute || e.push('minute')),
                  e
                );
              },
              isTouch: function () {
              ...
              calendarModule: function (t) {
                return t
                  ? (t instanceof e || (t = w.parent().children(t).first()), t.data(M))
                  : null;
              }
            },
            set: {
            ...
            selectDate: function (e, t) {
              var a = c.get.mode(),
                n =
                  t ||
                  'minute' === a ||
                  (f.disableMinute && 'hour' === a) ||
                  ('date' === f.type && 'day' === a) ||
                  ('month' === f.type && 'month' === a) ||
                  ('year' === f.type && 'year' === a);
              if (n) {
            ...
            helper: {
              sanitiseDate: function (e) {
                return e
                  ? (e instanceof Date || (e = v.date('' + e, f)), isNaN(e.getTime()) ? n : e)
                  : n;
              },
              ...
              isDateInRange: function (e, t, a, n) {
                ...
                return (
                  ...
                  !(
                    !e ||
                    (a && c.helper.dateDiff(e, a, t) > 0) ||
                    (n && c.helper.dateDiff(n, e, t) > 0)
                  )
                );
              },
              dateInRange: function (e, t, a) {
                ...
                var o = 'time' === f.type;
                return e
                  ? t && c.helper.dateDiff(e, t, 'minute') > 0
                    ? o
                      ? c.helper.mergeDateTime(e, t)
                      : t
                    : a && c.helper.dateDiff(a, e, 'minute') > 0
                      ? o
                        ? c.helper.mergeDateTime(e, a)
                        : a
                      : e
                  : e;
              },
            ...
            debug: function () {
              !f.silent &&
                ...
                (f.performance
                  ...
                  : ((c.debug = Function.prototype.bind.call(console.info, console, f.name + ':')),
                ...
                'string' == typeof t &&
                  ...
                  e.each(t, function (a, o) {
                    ...
                    if (e.isPlainObject(d[r]) && a != i) d = d[r];
                      ...
                      if (!e.isPlainObject(d[o]) || a == i)
                        return d[o] !== n ? ((l = d[o]), !1) : (c.error(b.method, t), !1);
                      d = d[o];
                    }
                  })),
                e.isFunction(l) ? (u = l.apply(r, a)) : l !== n && (u = l),
            ...
            s
              ? (F === n && c.initialize(), c.invoke(d))
              : (F !== n && F.invoke('destroy'), c.initialize()));
        }),
        o !== n ? o : r
      );
    }),
      ...
      parser: {
        date: function (t, a) {
          ...
          if (!g)
            for (
              c =
                e.inArray(a.text.am.toLowerCase(), h) >= 0 ||
                (!(e.inArray(a.text.pm.toLowerCase(), h) >= 0) && n),
                o = 0;
              o < m.length;
              o++
            ) {
              ...
              if (y.indexOf(':') >= 0) {
                if (u < 0 || l < 0) {
                  var v = y.split(':');
                  for (i = 0; i < Math.min(2, v.length); i++)
                    ((r = parseInt(v[i])),
                      isNaN(r) && (r = 0),
                      0 === i ? (u = r % 24) : (l = r % 60));
                }
          ...
          if (!f) {
            for (o = 0; o < h.length; o++) {
              ...
              if (!(D.length <= 0)) {
                for (
                  D = D.substring(0, Math.min(D.length, 3)), r = 0;
                  r < a.text.months.length;
                  r++
                ) {
                  var b = a.text.months[r];
                  if (
                    ((b = b.substring(0, Math.min(D.length, Math.min(b.length, 3))).toLowerCase()),
                    b === D)
                  ) {
                    s = r + 1;
                    break;
          ...
          (l < 0 && (l = 0),
            ...
            c !== n && (c ? 12 === u && (u = 0) : u < 12 && (u += 12)));
          var C = new Date(p, s - 1, d, u, l);
          return (
            (C.getMonth() === s - 1 && C.getFullYear() === p) || (C = new Date(p, s, 0, u, l)),
