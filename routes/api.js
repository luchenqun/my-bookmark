...
api.get('/notes', function(req, res) {
    ...
    if (params.shareNote) {
      ...
      .then((data) => res.send(`<body style="margin:0px;height:100%;"><head><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no, minimal-ui"></head><div style="height:100%;background-color:RGB(243,244,245); width:60%; margin-left:20%"><pre style="padding:10px; margin:0px;display: inline-block;text-align: left; font-size: 15px; font-family:italic arial,sans-serif;word-wrap: break-word;white-space: pre-wrap;">${data}</pre></div></body>`))
